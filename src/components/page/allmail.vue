<template lang="pug">
    el-card
        .clearfix(slot="header")
            el-row(:gutter="1")
                el-col( :span="19")
                    el-input(@keyup.enter.native="search" placeholder="请输标题" size="small" v-model="query.keyword" type="text" clearable style="width:290px")
                        el-button(slot="append" type="primary" icon='el-icon-search' @click="search")
                el-col(:span="2")
                    el-button( @click="handleCreate" size="small" icon="el-icon-plus" type="primary" style="float:right") 全体通知
                el-col(:span="2")
  ...
  data() {
    ...
    methods: {
        ...
        getData() {
            ...
            post('/apis/api/admin/mail/all',param).then(response => {
        ...
        async create(formName) {
            const set = this.$refs
                ...
                if (valid) {
                    post('/apis/api/admin/sendMail/all', this.form)
                            .then(() => {
        ...
        handleDeletes() {
            ...
            "提示",
            {
                ...
                _delete('/apis/api/admin/mail/deletes/0')
                .then(() => {
                    this.getData()
                    this.$notify({
                        title: "成功",
